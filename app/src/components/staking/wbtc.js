...
function BTC() {
  ...
  useEffect(() => {
    if (address !== "") {
      dispatch(Liquidity.getUserInfoDetails(3, address));
      dispatch(Liquidity.getPoolInfo(3));
    }
  }, [address]);
          ...
          if (parseFloat(_enteredValue) > parseFloat(_allowance)) {
            ...
            if (_approve) {
              ...
              let _addLiquidity = await StakingInstance.methods
                .addLiquidityToken(3, _enteredValue)
                .send({ from: address, value: 0 });

              if (_addLiquidity) {
                Error.toastifyMsg("info", "Liquidity Added Sucessfully");
                await dispatch(Liquidity.getUserInfoDetails(3, address));
                await dispatch(Liquidity.getPoolInfo(3));
    ...
    try {
      ...
      if (uid == undefined || uid == null) {
        ...
        let _wLiquidity = await StakingInstance.methods
          .withdrawLiquidity(3, uid) //hardcoded uId ,, fix in prod
          .send({ from: address });
          ...
          >
            ...
            <div className="col-md-3">
              <div className="aligncenter">
                <img src={btcLogo} className="inner-img" />
              </div>
            </div>
            <div
              id="contentBody"
            ...
            >
              <div id="contentText" className="ct__fs24">
                Provide liquidity in BTC/PXA @50% off
              </div>
            </div>
          </div>
                  ...
                  <Table responsive className="per70Card__addLiq">
                    <tbody>
                      ...
                      <tr>
                        ...
                        <td className="earn__tableCellValue">
                          {parseFloat(poolInfo.maxDeposit).toFixed(3)} BTC
                        </td>
                      </tr>
                      <tr>
