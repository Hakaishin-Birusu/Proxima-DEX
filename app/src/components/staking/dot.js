...
function Dot() {
  ...
  useEffect(() => {
    if (address !== "") {
      dispatch(Liquidity.getUserInfoDetails(4, address));
      dispatch(Liquidity.getPoolInfo(4));
    }
  }, [address]);
          ...
          if (parseFloat(_enteredValue) > parseFloat(_allowance)) {
            ...
            if (_approve) {
              ...
              let _addLiquidity = await StakingInstance.methods
                .addLiquidityToken(4, _enteredValue)
                .send({ from: address, value: 0 });

              if (_addLiquidity) {
                Error.toastifyMsg("info", "Liquidity Added Sucessfully");
                await dispatch(Liquidity.getUserInfoDetails(4, address));
                await dispatch(Liquidity.getPoolInfo(4));
    ...
    try {
      ...
      if (uid == undefined || uid == null) {
        ...
        let _wLiquidity = await StakingInstance.methods
          .withdrawLiquidity(4, uid) //hardcoded uId ,, fix in prod
          .send({ from: address });
          ...
          >
            ...
            <div className="col-md-3">
              <div className="aligncenter">
                <img src={dotLogo} className="inner-img" />
              </div>
            </div>
            <div
              id="contentBody"
            ...
            >
              <div id="contentText" className="ct__fs24">
                Provide liquidity in DOT/PXA @50% off
              </div>
            </div>
          </div>
                  ...
                  <Table responsive className="per70Card__addLiq">
                    <tbody>
                      ...
                      <tr>
                        ...
                        <td className="earn__tableCellValue">
                          {parseFloat(poolInfo.maxDeposit).toFixed(3)} DOT
                        </td>
                      </tr>
                      <tr>
