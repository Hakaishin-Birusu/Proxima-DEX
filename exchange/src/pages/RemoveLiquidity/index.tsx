...
export default function RemoveLiquidity({
  ...
  async function onRemove() {
    ...
    if (approval === ApprovalState.APPROVED) {
      // removeLiquidityETH
      if (oneCurrencyIsETH) {
        methodNames = ['removeLiquidityBNB']
        args = [
    ...
    // we have a signataure, use permit versions of remove liquidity
      ...
      if (oneCurrencyIsETH) {
        methodNames = ['removeLiquidityBNBWithPermit']
        args = [
          currencyBIsETH ? tokenA.address : tokenB.address,
          liquidityAmount.raw.toString(),
          amountsMin[currencyBIsETH ? Field.CURRENCY_A : Field.CURRENCY_B].toString(),
    ...
    const safeGasEstimates: (BigNumber | undefined)[] = await Promise.all(
      methodNames.map(methodName =>
        router.estimateGas[methodName](...args)
          ...
          .catch(error => {
            console.error(`estimateGas failed`, methodName)
            return undefined
          })
      )
    )
    ...
    if (indexOfSuccessfulEstimation === -1) {
      console.error('This transaction would fail. Previous transaction still pending.')
    } else {
      ...
      <>
        <RowBetween>
          <Text color={theme.text2} fontWeight={500} fontSize={16}>
            {'PLP ' + currencyA?.symbol + '/' + currencyB?.symbol} Burned
          </Text>
          <RowFixed>
